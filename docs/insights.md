...
3. **HTTP/1.1 keep-alive接続の失効**：1回の会話ターン（AG-UIの1リクエスト）でAnthropic APIを複数回（ツール呼び出しを挟んで）呼ぶ構成のため、呼び出しの間隔が空くとAnthropic側のエッジが先にkeep-alive接続を閉じることがあり、Envoyが失効した接続を掴んで再利用しようとすると`API Error: The socket connection was closed unexpectedly`になった（`kubectl logs -c app`で確認、Envoy自身のaccess_logには該当リクエストの記録が残らなかった＝レスポンスヘッダー到達前に切れた）。**対応**：ルートに`retry_policy: { retry_on: "reset,connect-failure,refused-stream", num_retries: 2 }`を追加した。Envoyの再試行はレスポンスヘッダー到達前の失敗のみが対象のため、二重実行の心配は無い。
   - frontend本実装後の実際の利用で、同じエラーメッセージだが**レスポンスヘッダー到達後・本文ストリーミング中**に切断される別パターンを複数回観測した（`retry_policy`の対象外）。**暫定対応**：アプリ層（`services/fraud-agent/src/app.ts`）で、同一`RunAgentInput`によるターン単位の1回自動リトライを追加した（[ADR 0037](adr/0037-fraud-agent-anthropic-stream-retry.md)）。
   - その後も再発したため`kubectl logs -c app`で真の一次エラーを確認したところ、実際は`API Error: 504 upstream request timeout`（Envoy自身が生成するエラー）であり、直後のSDK内部のセッション再開試行も同じ理由で失敗して最終的に`socket connection was closed unexpectedly`として露出していたと判明した。**原因**：`anthropic_gateway`ルートに`timeout`/`idle_timeout`の指定が無く、Envoyの既定route timeout（15秒）が適用されていた（`app_upstream`ルート等、他の全ホップには`timeout: 0s`/`idle_timeout: 300s`が設定済みだったが、このルートだけ抜けていた）。まとまった量のテキストを生成する応答は日常的に15秒を超えるため、ほぼ確実に再現していた。**対応**：このルートにも`timeout: 0s`/`idle_timeout: 300s`を追加した（[ADR 0038](adr/0038-fraud-agent-anthropic-route-timeout.md)）。修正後、`scripts/verify-hop.sh`のログイン手法を使ったアドホック検証で、口座789・456とも`RUN_ERROR`無しで`RUN_FINISHED`まで到達すること（各45秒・35秒）を確認した。
