...
このトークンには、アナリストの担当地域・権限レベルは一切含まれない。これらはanalyst-attribute-serviceが保持する外部属性であり、必要になった都度、後続のToken Exchangeの先で照会される（表5）。ログイントークン自身に埋め込まない理由は、単に「変更のたびにトークンを再発行する手間を省くため」ではなく、以下の3点による。

- **失効の即時性（最も本質的な理由）**：Keycloakが発行するJWTは署名済みで検証はローカル・ステートレスに行われるため、発行後は取り消せない。もし担当地域・権限レベルをクレームとして埋め込むと、異動やインシデント対応による緊急の権限剥奪が発生しても、既に発行済みのトークンは有効期限が切れるまで古い権限情報を正当なものとして各サービスに信じ込ませ続ける。これは利便性の問題ではなく、変更が即座に効かないというセキュリティ上の窓（revocation latency）の問題である。account-serviceが毎回analyst-attribute-serviceへ生で照会する設計なら、常にその時点の最新状態を参照するため、この窓自体が発生しない
- **業務データの露出範囲の最小化**：JWTは署名されているだけで暗号化されていない。`aud`によるaudience制限は「このリソースサーバーはこのトークンを受理すべきだ」という検証時の受理条件でしかなく、意図した宛先以外に内容を隠す機密性は提供しない。そのため、たとえ「account-service宛ての交換でしか付与しない」ようにクレームを設計しても、トークンを発行するfrontend自身や、中継するfraud-mcp-server、各ホップのアクセスログ・OTelトレースには中身がそのまま見えてしまう。一方analyst-attribute-serviceへの照会はaccount-serviceとの間だけで完結し、frontend・fraud-mcp-serverの手を一度も経由しない。前者が「見ないでね」という約束に頼るのに対し、後者は届く経路自体を作らないという構造的な違いがある
- **正典（source of truth）の一本化**：クレームに載せるにはKeycloak側にもこのデータを複製・同期する必要が生じ、人事異動をどちらの系（HR/業務システムかKeycloakか）が正として反映するのかが曖昧になり、同期漏れによる齟齬の火種になる。analyst-attribute-serviceだけを正典にすればこの複製自体が発生しない

これは本システム固有の判断ではなく、ABAC（属性ベースアクセス制御）で一般的に推奨される設計（属性の発行源をトークン発行から分離するモデル）に沿ったものである。
