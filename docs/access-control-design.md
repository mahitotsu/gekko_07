...
- **業務データの露出範囲の最小化**：クレームに載せると、そのトークンが通過する全てのホップ（fraud-mcp-server等、本来この情報を必要としない中継サービスも含む）に担当地域・権限レベルが見えてしまう。account-serviceだけが照会できる別サービス（表3）に閉じ込める方が、必要な箇所にだけ露出を限定できる
