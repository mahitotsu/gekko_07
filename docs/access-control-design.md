...
このトークンには、アナリストの担当地域・権限レベルは一切含まれない。これらはanalyst-attribute-serviceが保持する外部属性であり、必要になった都度、後続のToken Exchangeの先で照会される（表5）。クレームにするか業務データとして外部化するかは、オーナーシップ・機密性・鮮度要件の3軸で判断する（[ADR 0006](adr/0006-claim-vs-external-attribute-criteria.md)）。担当地域・権限レベルはこの3軸全てで「外部化すべき」側に該当する。

- **オーナーシップ**：正典は人事上の配属情報であり、業務ドメイン（analyst-attribute-service）が正典。Keycloak側にも複製すると、人事異動の反映元がどちらなのか曖昧になり同期漏れの火種になる
- **機密性**：JWTは署名されているだけで暗号化されていない。`aud`によるaudience制限は「このリソースサーバーが受理すべきか」という検証時の受理条件でしかなく、意図した宛先以外に内容を隠す機密性は提供しない。たとえ「account-service宛ての交換でしか付与しない」ようにクレームを設計しても、トークンを発行するfrontend自身や、中継するfraud-mcp-server、各ホップのアクセスログ・OTelトレースには中身がそのまま見えてしまう。一方analyst-attribute-serviceへの照会はaccount-serviceとの間だけで完結し、frontend・fraud-mcp-serverの手を一度も経由しない
- **鮮度要件**：トークン失効自体の手段（イントロスペクション、失効リスト、短いTTL）はOAuthに標準で存在するが、これらが保証するのは主に「トークン全体がまだ有効か」であり、クレームとして埋め込んだ個々の値を都度最新化するものではない。担当地域・権限レベルをクレームとして埋め込むと、異動やインシデント対応による緊急の権限剥奪が発生しても、値自体はAS側が発行元へ再照会する仕組みを別途持たない限りトークンの有効期限まで古いまま扱われ続ける。account-serviceが毎回analyst-attribute-serviceへ生で照会する設計なら、AS側の追加実装なしに常にその時点の最新状態を参照できる
