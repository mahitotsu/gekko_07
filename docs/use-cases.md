...
6. account-service: 東京の standard 口座の不審取引を返す（access-control-design.md 表5）
...
9. frontend: 自身のログイントークン（aud=frontend）を`subject_token`に別のToken Exchangeを実行（audience=account-service, scope=account:freeze）し、そのトークンでaccount-serviceの凍結APIを呼ぶ
...
3. account-service: scope=account:transactを確認 → 許可。analyst-attribute-serviceへの照会は発生しない（access-control-design.md 表4）
```
