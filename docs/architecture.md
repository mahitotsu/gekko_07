...
- 実装順序：fraud-mcp-server→account-serviceの`account:read`/`account:propose`（パターン①）、fraud-detection-engine→account-serviceの`account:freeze`（パターン②、client_credentials）の2ホップで先行検証**済み**（`k8s/ext-authz/`・`k8s/account-service/`・`k8s/fraud-mcp-server/`・`k8s/fraud-detection-engine/`、`scripts/verify-hop.sh`。詳細は[insights.md](insights.md)）。残りのホップ（frontend方向、account-service→analyst-attribute-service等）は未検証（[backlog.md](backlog.md)参照）
- パターン①（fraud-mcp-server→account-service）のToken Exchange実行主体は、[ADR 0019](adr/0019-ext-authz-identity-gap-and-spiffe-jwt-svid-auth.md)で共有の`ext-authz-service`からfraud-mcp-server自身のPod内`token-exchange`サイドカーへ移し、クライアント認証もclient_secretからSPIRE発行JWT-SVID（KeycloakネイティブのSPIFFE対応）へ変更した。パターン②（`ext-authz-service-cc`）は本変更のスコープ外で、従来通りclient_secretを使う共有インスタンスのまま
