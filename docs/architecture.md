...
- 実装順序：fraud-mcp-server→account-serviceの`account:read`/`account:propose`（パターン①）、fraud-detection-engine→account-serviceの`account:freeze`（パターン②、client_credentials）、account-service→analyst-attribute-serviceの`analyst:read`（表3）、fraud-agent→fraud-mcp-serverの`account:read`（[ADR 0023](adr/0023-fraud-agent-fraud-mcp-server-hop.md)）の4ホップで先行検証**済み**（`k8s/account-service/`・`k8s/fraud-mcp-server/`・`k8s/fraud-detection-engine/`・`k8s/analyst-attribute-service/`・`k8s/fraud-agent/`、`scripts/verify-hop.sh`。詳細は[insights.md](insights.md)）。frontend自体が未実装のため、frontend→account-service・frontend→fraud-agentと、fraud-agent自身のingress側（frontend→fraud-agentのmTLS/JWT検証）は未検証（[backlog.md](backlog.md)参照）
