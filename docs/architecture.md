...
- 実装順序：fraud-mcp-server→account-serviceの`account:read`/`account:propose`（パターン①）、fraud-detection-engine→account-serviceの`account:freeze`（パターン②、client_credentials）の2ホップで先行検証**済み**（`k8s/account-service/`・`k8s/fraud-mcp-server/`・`k8s/fraud-detection-engine/`、`scripts/verify-hop.sh`。詳細は[insights.md](insights.md)）。残りのホップ（frontend方向、account-service→analyst-attribute-service等）は未検証（[backlog.md](backlog.md)参照）
- パターン①（fraud-mcp-server→account-service）・パターン②（fraud-detection-engine→account-service）とも、Token Exchange/client_credentials実行主体を共有の`ext-authz-service`(-cc)から各呼び出し元自身のPod内サイドカーへ移し、クライアント認証もclient_secretからSPIRE発行JWT-SVID（KeycloakネイティブのSPIFFE対応）へ変更した（[ADR 0019](adr/0019-ext-authz-identity-gap-and-spiffe-jwt-svid-auth.md)・[ADR 0020](adr/0020-fraud-detection-engine-identity-gap-and-client-credentials-federated-jwt.md)）。これにより共有`ext-authz-service`インスタンス方式（`k8s/ext-authz/`）は全廃した
