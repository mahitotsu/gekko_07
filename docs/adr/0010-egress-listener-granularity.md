...
- **[0030](0030-fraud-agent-implementation.md)で3つ目のパターンが新設された**：fraud-agentがClaude Agent SDK経由で呼ぶAnthropic API向けegressは、Keycloak登録済みaudience宛てを前提とする本ADRの①②いずれにも該当しない。appは内部専用の別名（`anthropic-gateway`、`ANTHROPIC_BASE_URL`環境変数）経由で接続し、Envoyが公開CA検証でTLSを終端して実際の`api.anthropic.com`へ再接続する、本ADRの2パターンとは別枠の構成を採った（`k8s/fraud-agent/envoy-configmap.yaml`のegress:80、anthropic_gateway仮想ホスト）
