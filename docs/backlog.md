...
- **素通し・トークンを値として取得する2パターンの実機検証**：[ADR 0010](adr/0010-egress-listener-granularity.md)でEnvoy標準機能のみで実現できる設計（③はext_authzを呼ばない単純プロキシ、④`direct_response`+`allowed_client_headers_on_success`）まで固めたが、実機での動作確認はこれから（特に④のext_authz+direct_responseの組み合わせは実機で挙動を要確認）。パターン①（Token Exchange、fraud-mcp-server→account-service）・パターン②（client_credentials、fraud-detection-engine→account-service）は実機検証済み（[insights.md](insights.md)参照）
- **ext-authzサービスの呼び出し元汎用化（複数クライアント対応）**：[k8s/ext-authz/](../k8s/ext-authz/)は現状、呼び出し元ごとに固定資格情報を持つ別インスタンス（`ext-authz-service`=fraud-mcp-server固定のToken Exchange、`ext-authz-service-cc`=fraud-detection-engine固定のclient_credentials）を並べる形で対応している。残りのホップ（frontend→account-service/fraud-mcp-server、account-service→analyst-attribute-service）へ横展開する際、1インスタンスが呼び出し元ごとに資格情報を動的に切り替える仕組みが要るか、インスタンスを増やし続けるかを判断する
