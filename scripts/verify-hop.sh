#!/bin/bash
# fraud-mcp-server→account-serviceの1ホップ先行検証(ADR 0002/0009/0010)。
...
FRAUD_MCP_POD=$(kubectl -n "$NAMESPACE" get pod -l app=fraud-mcp-server -o jsonpath='{.items[0].metadata.name}')
...
ACCOUNT_POD_IP=$(kubectl -n "$NAMESPACE" get pod -l app=account-service -o jsonpath='{.items[0].status.podIP}')
