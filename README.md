...
- [x] fraud-mcp-server→account-serviceのToken Exchangeクライアント認証をclient_secretからSPIRE発行JWT-SVID（KeycloakネイティブのSPIFFE対応）へ移行し、Token Exchange実行主体を共有ext-authz-serviceからfraud-mcp-server自身のPod内サイドカーへ移した（[ADR 0019](docs/adr/0019-ext-authz-identity-gap-and-spiffe-jwt-svid-auth.md)。Keycloakが検証する身元と主張するclient_idの不一致という身元検証上のギャップを解消。他パターンへの横展開は未着手、docs/backlog.md参照）
