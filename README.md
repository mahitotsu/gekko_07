...
OAuth 2.0 Token Exchange (RFC 8693) をEnvoyサイドカー（ext_authz）に実装し、マイクロサービス群とAIエージェントが連携するローカル実行可能なサンプル。**現在はインフラ・セキュリティ層（Token Exchange・SPIFFE/SPIRE mTLS・NetworkPolicy・監査ログ集約）をスタブサービス相手に実機検証済みの段階**（各サービスの本実装は未着手。frontend・fraud-agent（AIエージェント）は検証用スタブのみ実装済み）。目的・背景・要求水準は[docs/requirements.md](docs/requirements.md)を参照。
...
- [ ] 各サービスの実装（本実装。account-service・fraud-mcp-server・fraud-detection-engine・analyst-attribute-service・fraud-agent・frontendいずれも現状はEnvoy/SPIRE検証用スタブのみで、frontendのログインも簡易ログイン（ROPC）で代用中。詳細は[docs/services.md](docs/services.md)参照）
...
make deploy             # アプリ層（Postgres・SPIRE・Keycloak・edge-proxy・監査ログ集約基盤・NetworkPolicy）を再デプロイ（クラスタは起動済み前提）
make undeploy           # アプリ層だけを削除（クラスタは残す）
make stop               # クラスタを停止（状態は保持）
make start              # 停止したクラスタを再開
make down               # クラスタを完全削除

make deploy-verify-hop  # Envoy/ext_authzの全6ホップ先行検証用スタブ一式をデプロイ（make deploy実行済み前提）
